...
std::vector<std::string> words = {"piwo", "armata", "autostrada", "policja", "dzielnica", "akwarium"};
    ...
    while (true)
    {
        ...
        if (secret_word.find(guess) != std::string::npos)
        {
            ...
            if (secret_word == guess_word)
            {
                std::cout << hangman_art[try_no] << "\n";
                std::cout << guess_word << "\n";
                std::cout << "Wygrales! Ukryte slowo to " << secret_word << "\n";
                break;
            }
        } else {
            ++try_no;
        ...
        if (try_no >= 6)
        {
            std::cout << hangman_art[try_no] << "\n";
            std::cout << guess_word << "\n";
            std::cout << "Przegrales! Ukryte slowo to " << secret_word << "\n";
            break;
        }
    }
}
